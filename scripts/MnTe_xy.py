...
def workflow(model,id):
    ...
    afm.plot('electron',ylim=[9,13],save=True,figsize=(4,6),save_name='plot_KGK',title=fr"$\epsilon$={epsilon} a={a} c/a={ac}") #plot electron bands
    path = ['M','GAMMA','M'] #choose corners
    afm.band_points(path,number=50,points=points) #define path
    afm.calculate('bands',pp_core=8)
    afm.plot('electron',ylim=[9,13],save=True,figsize=(4,6),save_name='plot_MGM',title=fr"$\epsilon$={epsilon} a={a} c/a={ac}") #plot electron bands
    # path = ["M'","GAMMA'","M'"] #choose corners
